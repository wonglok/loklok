...
// ---------------------------------------------------------------------------
// Raw GLB builder (produces a valid glTF 2.0 binary for gltf-transform to parse)
// ---------------------------------------------------------------------------
interface MeshPrim {
  obj: BlenderObject;
  geo: GeoBuffer;
...
// ---------------------------------------------------------------------------
// Optimized GLB export (gltf-transform pipeline)
// ---------------------------------------------------------------------------
let _ready = false;
...
export async function downloadGLB(): Promise<void> {
  ...
  try {
    ...
    const doc = await io.readBinary(rawGlb as Buffer);
